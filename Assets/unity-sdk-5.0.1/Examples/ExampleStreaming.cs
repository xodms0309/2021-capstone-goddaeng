    ...
    public class ExampleStreaming : MonoBehaviour
    {
        ...
        void Update()
        {
            if (timestart == true)
            {
                ...
                sit = true;
                    if (fTickTime>=3f&&sit&&!sitted)
                    {
                        sitted = true;
                        anim.Play("CorgiSitToIdle 0");
                    }
                }
            }
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        if (res.final&&alt.transcript.Contains("앉아")) {
                            anim.Play("CorgiIdleToSit", -1, 0f);
                            timestart=true;
                        }
                        if (res.final && alt.transcript.Contains("안녕"))
                        {
                            anim.Play("CorgiIdleBarking", -1, 0f);
