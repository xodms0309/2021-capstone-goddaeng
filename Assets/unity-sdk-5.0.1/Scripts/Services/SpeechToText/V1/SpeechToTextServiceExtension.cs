...
using CloudUtility = IBM.Cloud.SDK.Utilities.Utility;
        ...
        private SpeakerRecognitionEvent ParseSpeakerRecognitionResponse(IDictionary resp)
        {
            ...
            try
            {
                ...
                foreach (var r in iresults)
                {
                    ...
                    result.confidence = CloudUtility.StringToDouble(iresult["confidence"].ToString());
                    result.final = (bool)iresult["final"];
                    result.from = CloudUtility.StringToDouble(iresult["from"].ToString());
                    result.to = CloudUtility.StringToDouble(iresult["to"].ToString());
                ...
                foreach (var r in iresults)
                {
                    ...
                    if (iwordAlternatives != null)
                    {
                        ...
                        foreach (var w in iwordAlternatives)
                        {
                            ...
                            if (iwordAlternative.Contains("start_time"))
                                wordAlternativeResults.start_time = CloudUtility.StringToDouble(iwordAlternative["start_time"].ToString());
                            if (iwordAlternative.Contains("end_time"))
                                wordAlternativeResults.end_time = CloudUtility.StringToDouble(iwordAlternative["end_time"].ToString());
                            if (iwordAlternative.Contains("alternatives"))
                            {
                                ...
                                foreach (var a in iwordAlternativeResult)
                                {
                                    ...
                                    if (ialternative.Contains("confidence"))
                                        wordAlternativeResult.confidence = CloudUtility.StringToDouble(ialternative["confidence"].ToString());
                    ...
                    if (ialternatives != null)
                    {
                        ...
                        foreach (var a in ialternatives)
                        {
                            ...
                            if (ialternative.Contains("confidence"))
                                alternative.confidence = CloudUtility.StringToDouble(ialternative["confidence"].ToString());
                            ...
                            if (ialternative.Contains("timestamps"))
                            {
                                ...
                                for (int i = 0; i < itimestamps.Count; ++i)
                                {
                                    ...
                                    ts.Start = CloudUtility.StringToDouble(itimestamp[1].ToString());
                                    ts.End = CloudUtility.StringToDouble(itimestamp[2].ToString());
                                    timestamps[i] = ts;
                                }
                            ...
                            if (ialternative.Contains("word_confidence"))
                            {
                                ...
                                for (int i = 0; i < iconfidence.Count; ++i)
                                {
                                    ...
                                    wc.Confidence = CloudUtility.StringToDouble(iwordconf[1].ToString());
                                    confidence[i] = wc;
                                }
                        ...
                        foreach (string keyword in Keywords)
                        {
                            if (iKeywords[keyword] != null)
                            {
                                ...
                                foreach (var k in iKeywordList)
                                {
                                    ...
                                    keywordResult.confidence = CloudUtility.StringToDouble(iKeywordDictionary["confidence"].ToString());
                                    keywordResult.end_time = CloudUtility.StringToDouble(iKeywordDictionary["end_time"].ToString());
                                    keywordResult.start_time = CloudUtility.StringToDouble(iKeywordDictionary["start_time"].ToString());
